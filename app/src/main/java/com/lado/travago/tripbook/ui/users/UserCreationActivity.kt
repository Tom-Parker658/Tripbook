...
import com.lado.travago.tripbook.viewmodel.UserCreationViewModelFactory
import com.lado.travago.tripbook.viewmodel.admin.UserCreationViewModel
import com.lado.travago.tripbook.viewmodel.admin.UserCreationViewModel.FieldTags
...
class UserCreationActivity : AppCompatActivity() {
     ...
     * If the received photoUri is not null, we convert the uri to a bitmap and set its value to that of [UserCreationViewModel.photoField]
    ...
    private fun getIntentData(): Pair<String?, String?> {
        ...
        val agencyId = intent.getStringExtra(AgencyRegistrationActivity.KEY_AGENCY_ID)

        Log.i("UserCreationActivity", "agencyName=$agencyName, path=$agencyFirestorePath")
    ...
    /**
     * Observe the [UserCreationViewModel.loading] live data to know when a process is actually in the loading state
